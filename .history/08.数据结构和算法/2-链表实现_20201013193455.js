class ListNode{
    constructor(key){
        this.next=null;
        this.key=ley;
    }
}

class List{
    constructor(){
        this.head=null;
        this.length=0;
    }
    static createNode(key){
        return new ListNode(key);
    }

    //往头部插入数据
    insert(node){
        //如果头部有节点
        if(this.head){
            node.next=this.head;
        }else{
            node.next=null;
        }
        //无论如何都成了头部节点
        this.head=node;
        this.length++;
    }

    find(key){//查找结点
        if (this.length == 0) {
            throw 'the List is null';
        }

        let node = this.head;//从头节点开始向后查找
        while(node.key!=key&&node!=null){
            node=node.next;
        }
        if(node==null) throw 'node is not in the List';
        return node;
    }

    delete(node){
        if(this.length==0){
            throw 'the List is null';
        }

        let pre = this.head;
        while(node!=pre.next&&pre!=null){
            pre = pre.next
        }
        if(pre.next){
            pre.next = node.next;
            this.length--;
        }else{
            throw 'the node no exits'
        }

    }
}

//双向链表
class DoubleNode{
    constructor(key){
        this.pre=null;
        this.key=key;
        this.next=null;
    }
}
class DoubleLinkedList{
    constructor() {
        this.head=null;
    }
    static createNode(key){
        return new DoubleNode(key);
    }
    insert(node){

    }
    search(key){

    }
    delete(node){

    }
}


//另一种链表的实现
class LinkedLisr{
    constructor(value){
        this.head={value,newt:null};//头部
        this.tail=this.head;//尾部
    }

    insert(value){//从尾部插入
        const node={value,next:null};//根据新传入的值创建一个节点
        this.tail.next = node;//尾部的next指向node
        this.tail=node;//node成为新的尾部
    }

    find(value){//输入值进行查找
        if(this.head.value==value){
            return this.head;
        }
        let pre = this.head;
        let cur=pre.next;
        while(cur.value!=value&&cur!=null){
            pre = cur;
            cur=cur.next;
        }
        if(cur){
            return cur;
        }else{
            return false;
        }
    }

    removeNode(value){
        if (this.head.value == value) {
            this.head = this.head.next;
            return true;
        }
        let pre = this.head;
        let cur = pre.next;
        while (cur.value != value && cur != null) {
            pre = cur;
            cur = cur.next;
        }
        if (cur) {
            pre.next=cur.next;
            return true;
        } else {
            return false;
        }
    }
}

//哈希列表
class HashTable{
    constructor(size){
        //定义哈希表的大小，将在哈希函数中使用
        this.size=size;
        this.storage=[];
    }
    insert(key,value){
            //获取数组中的索引
            const index=this.myHashingFunction(key,this.size);
            //处理冲突-哈希函数为不同的键返回相同的索引
            //在复杂的哈希函数中，很可能发生
            if(!this.storage[index]){//如果没有存储这个索引值,那么为这个索引配一个数组
                this.storage[index] = [];
            }
        //push 新的键值对
        this.storage[index].push([key,value]);
    }
    get(key){
        //首先获取key的索引
        const index = this.myHashingFunction(key, this.size);
        //一个索引处可能有多个数组（不太可能）
        let arrAtIndex = this.storage[index];
        if (arrAtIndex) {
            //遍历该索引处的所有数组
            for (let i in arrAtIndex) {
                //检查key是否于参数key匹配
                if (arrAtIndex[i][0] === key) {
                    return arrAtIndex[i];
                    break;
                }
            }
        }
    }
    remove(key){
        //首先获取key的索引
        const index = this.myHashingFunction(key,this.size);
        //一个索引处可能有多个数组（不太可能）
        let arrAtIndex = this.storage[index];
        if(arrAtIndex){
            //遍历该索引处的所有数组
            for(let i in arrAtIndex){
                //检查key是否于参数key匹配
                if(arrAtIndex[i][0]===key){
                    delete arrAtIndex[i];
                    break;
                }
            }
        }
    }

    //计算散列密钥的方式
    myHashingFunction(str,n){
        let sum=0;
        for(let i in str){
            sum+=str.charCodeAt(i)*3;//charCodeAt 返回字符串指定位置的unicode编码
        }
        return sum%n;
    }
}

const ht = new HashTable(5);
ht.insert('a',1);
ht.insert('n',5)
console.log(...ht.storage);
ht.remove('a');
console.log(...ht.storage);

//二叉搜索树
class Node{//二叉树的节点类
    constructor(value,left=null,right=null){
        this.value=value;
        this.left=left;
        this.right=right;
    }
}
class Tree{
    constructor(){
        this.root=null;//二叉树本书就一个根节点属性，其他的都是节点的属性
    }
    add(value){
        if(!this.root){
            this.root=new Node(value);
            return;
        } 
        let cur=this.root;
        while(true){
            //传入的小于当前的节点值，就向左
            if(value<cur.value){

                if(cur.left){//存在左子树就接着循环
                    cur=cur.left;
                }else{//不存在左子树，我就当作左子树
                    cur.left=new Node(value);
                    return;
                }

            }else{//这里的都是比当前节点大，到右边的

                if(cur.right){//存在右子树，则下面的大小位置，移动一位接着比较
                    cur=cur.right;
                }else{
                    cur.right = new Node(value);
                    return;
                }
            }
        }
        
    }
    contains(value){
        let cur=this.root;
        while(true){
            if(cur.value==value){//如果是当前值，退出循环返回true
                return true;
            }

            if(cur.value>value){//不是的话，判断参数与当前值的大小
                if(cur.left){//小的来左边找，有左子树的移到左子树进行下一轮比较
                    cur=cur.left;
                }else{//没有左子树就是找不到，返回false
                    return false;
                }
            }else{
                if(cur.right){
                    cur=cur.right;
                }else{
                    return false
                }
            }
        }
    }
}

let t = new Tree();
t.add(2);
t.add(5);
t.add(1);
console.log(t);
console.left(t.contains(1));