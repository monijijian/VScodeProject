...
/*这里涉及到javaScript的eventLoop机制
js中有三种队列存储事件：任务栈、任务队列、nextTick；优先级：任务栈>nextTick>任务队列
任务栈中的先执行，空了之后执行nextTick，最后执行任务队列中的事件。

普通的语句、new的promise中的语句、async中await修饰的语句放进任务栈
promise的then、async中没await修饰的语句  放进任务队列
Process.nextTick语句放进专属的 nextTick队列

定时函数setTimeout，setInterval的作用是在指定时间后将内部语句放在下一次循环的宏任务队列
多个promise的then会交替的添加到任务队列中
*/